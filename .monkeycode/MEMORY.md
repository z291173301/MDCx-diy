...
- Instructions:
  ...
  - **检查纪律**：每次代码改动后跑 `uv run quick-check`；提交前跑 `uv run check --skip-hook-install`。仅改 `docs/*.md` 或本文件时只需 `git diff --check`。全绿判定 = 退出码 0 + `grep -E "\.py:[0-9]+: error|Found [0-9]+ error"` 无输出（CI 连挂三次的教训：mypy 输出可被 tail 截断）。**`ruff check` 过 ≠ `ruff format --check` 过——改 .py 必须 `ruff format` 落地**（2026-09-09 四次 CI 挂全是 format 漂移）。`scripts/` 也在 check 范围。**xlsx 校验按路径跳过（2026-09-21）**：`check_actor_db` / `check_info_db` 只在对应出厂库或校验脚本有改动时跑（工作区 + 相对上游 diff）；`check_ui_layout` 已移出门禁（warning 不阻断，结构锁在 `test_ui_structure.py`），手工诊断仍可 `uv run python -m scripts.check_ui_layout`。**pre-push 钩子三注意（2026-09-20 实证）**：①`hooksPath` 是本地 git config，**环境重置后静默失效**——而日常跑的 `check --skip-hook-install` 恰恰跳过自动配置钩子那步，两者叠加导致失效无人察觉；重置后先 `git config core.hooksPath` 验证，为空则 `git config core.hooksPath .githooks` 补配置；②钩子生效时日常流程=「quick-check → commit → push（钩子自动全量）」，**不要再手动多跑一次全量**（省 3 分钟）；③钩子失效期间推送前必须手动全量 check 兜底；④`.pre-commit-config.yaml` 为历史遗留、不依赖。
